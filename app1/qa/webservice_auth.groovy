...
pipeline {
    ...
    stages {
        ...
        stage('Preparing config files') {
            steps {
                sh 'mv deploy_projects/app1/__projects/webservice_auth/params-qa.yml webservice_app/env/params-qa.yml'
                sh 'mv deploy_projects/app1/__projects/webservice_auth/params-qa-vpc.yml webservice_app/env/vpc.yml'
                sh 'mv deploy_projects/app1/__projects/webservice_auth/.mocharc.js webservice_app/.mocharc.js'
                sh 'rm -rf deploy_projects'

                sh 'cat webservice_app/env/vpc.yml >> webservice_app/env/params-qa.yml'
        ...
        stage('Replace params credentials') {
            steps {
                script {
                    modules._general.replaceParams('webservice_app/env/params-qa.yml')
                }
            }
        }
